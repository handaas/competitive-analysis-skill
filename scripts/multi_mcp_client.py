...
The competitive-analysis scenario needs cross-domain data from 7 MCP servers:
  enterprise / enterprise-operation / patent / trademark / bidding /
  news / qualification.
...
Usage (from compose layer)::
    ...
    async with MultiMcpClient() as client:
        stats = await client.call("patent", "patent_bigdata_patent_stats", {...})
...
SERVER_REGISTRY: Dict[str, str] = {
    ...
    "qualification": "qualification-mcp-server",
}
...
REMOTE_URL_ENV: Dict[str, str] = {
    ...
    "news": "NEWS_MCP_URL",
    "qualification": "QUALIFICATION_MCP_URL",
}


# Skill-level MCP token (shared across all domains in this skill)
SKILL_MCP_TOKEN = "COMPETITIVE_ANALYSIS_MCP_TOKEN"
...
class MultiMcpClient:
    ...
    async def ensure_session(self, domain: str) -> Any:
        ...
        if not conn:
            raise MultiMcpError(
                f"未配置 MCP 连接 [{domain}]。请设置 COMPETITIVE_ANALYSIS_MCP_TOKEN 或 {REMOTE_URL_ENV.get(domain, 'HANDAAS_MCP_URL')}（Remote），"
