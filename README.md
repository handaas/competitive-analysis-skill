...
一个可被本地智能体使用的 HandaaS 场景研判 Skill。用户只需要说"使用竞争分析，对比评估广州探迹科技有限公司的技术实力与市场定位"，智能体会自动连接多个 MCP 服务、获取原始数据，通过证据评分模型在多个维度上交叉评分，输出综合画像报告（HTML + Markdown + JSON + PDF 四件套），内嵌 ECharts 5.x 雷达图与仪表盘，离线可打开。

> 直连 7 个 MCP server（工商 / 经营 / 专利 / 商标 / 招投标 / 舆情 / 资质），从创新商业化能力、市场定位、品牌专利协同等维度交叉评分，输出竞争定位报告。
...
```text
请帮我安装并调试 HandaaS competitive-analysis-skill：先完整读取 https://github.com/handaas/competitive-analysis-skill 这个项目的 README.md，再克隆仓库并按 README 自动完成安装、配置文件创建、校验、模拟运行验证和示例报告生成。若当前工具支持 Skill 目录安装，就把 competitive-analysis-skill/ 安装到对应 skills 目录；若不支持，就把该仓库作为本地工具包使用。请提醒我只在本地配置自己的企业数据接口参数或在线 MCP token，不要提交凭证；除非需要真实接口密钥，否则不要中断询问。
```
...
本场景直连以下 7 个 MCP server 获取数据。在 [HandaaS 平台](https://www.handaas.com/) 创建 MCP 对接器时，选择以下所需的在线服务，所选服务共用同一个 token。
...
```bash
export COMPETITIVE_ANALYSIS_MCP_TOKEN="<your-token>"
...
export QUALIFICATION_MCP_URL="https://mcp.handaas.com/qualification/qualification_bigdata"
```

Windows PowerShell：

```powershell
$env:COMPETITIVE_ANALYSIS_MCP_TOKEN = "<your-token>"
...
$env:QUALIFICATION_MCP_URL = "https://mcp.handaas.com/qualification/qualification_bigdata"
```

将 `<your-token>` 替换为你在 HandaaS 平台获取的实际 token。token 由 `COMPETITIVE_ANALYSIS_MCP_TOKEN` 统一携带，URL 中无需重复填写。
